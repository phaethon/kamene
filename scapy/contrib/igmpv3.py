...
# Usage examples:
#  General query:
#   Ether() / Dot1Q() / IP() / IGMP() / IGMPv3mq()
#  Membership report:
#   Ether() / Dot1Q() / IP() / IGMP() / IGMPv3mr() / IGMPv3gr(rtype=6, maddr='255.255.255.255')
#  Source-specific:
#   pkt[IGMPv3gr].srcaddrs = ['1.2.3.4', '5.6.7.8']


#import sys, socket, struct, time
from scapy.all import *

print("IGMPv3  is still under development - Dec 2017")


class IGMPv3(Packet):
    """Basic class for IGMP Message v3.
    """
    name = "IGMPv3"
    ...
    #if type = 0x30 (Multicast Router Advertisement), the next fields are
...
class IGMPv3mq(Packet):
    ...
    Add payload of IGMPv3 when type=0x11.
    """
    name = "IGMPv3mq"
...
class IGMPv3gr(Packet):
    """IGMP Group Record extension for IGMPv3 Membership Report.
...
class IGMPv3mr(Packet):
    ...
    """
    name = "IGMPv3mr"
    fields_desc = [ByteField("res1", 0),
                   ByteField("res2", 0),
                   FieldLenField("numgrp", None, count_of="records"),
                   PacketListField("records", [], IGMPv3gr, count_from="numgrp")
                   ]
...
bind_layers(IP, IGMPv3, frag=0, proto=2, ttl=1, tos=0xc0, options=[IPOption_Router_Alert(length=4, copy_flag=1, optclass=0, option=20, alert=0)])
