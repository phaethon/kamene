...
def get_temp_file(keep=False, autoext=""):
    import tempfile
    fd, fname  = tempfile.mkstemp(suffix = ".scapy3k" + autoext)
    os.close(fd)
    if not keep:
...
def do_graph(graph,prog=None,format='png',target=None,string=False,options=None, figsize = (12, 12), **kargs):
    ...
    from scapy3k.arch import NETWORKX
    if NETWORKX:
...
class PcapReader(RawPcapReader):
    ...
    def read_all(self,count=-1):
        ...
        import scapy3k.plist
        return scapy3k.plist.PacketList(res,name = os.path.basename(self.filename))
    def recv(self, size=MTU):
