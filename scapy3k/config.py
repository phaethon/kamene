...
from .data import *
import scapy3k.base_classes
import scapy3k.themes
...
class ConfigFieldList:
    ...
    def __contains__(self, elt):
        if isinstance(elt, scapy3k.base_classes.Packet_metaclass):
...
class Num2Layer:
    ...
    def __getitem__(self, item):
        if isinstance(item, scapy3k.base_classes.Packet_metaclass):
...
class Conf(ConfClass):
    """This object contains the configuration of scapy3k.
...
"""
    ...
    auto_fragment = 1
    debug_dissector = 0
    color_theme = Interceptor("color_theme", scapy3k.themes.NoTheme(), _prompt_changer)
    warning_threshold = 5
    prog = ProgPath()
    resolve = Resolve()
    noenum = Resolve()
...
if not Conf.ipv6_enabled:
    log_scapy.warning("IPv6 support disabled in Python. Cannot load scapy3k IPv6 layers.")
