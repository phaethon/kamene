...
from scapy3k.error import warning
import scapy3k.config
import scapy3k.utils
import scapy3k.utils6
import scapy3k.arch

scapy3k.config.conf.use_winpcapy = True
scapy3k.config.conf.use_netifaces = True
scapy3k.config.conf.use_dnet = True
...
def read_routes():
    ...
    for l in f.split('\n'):
        ...
        if dest == "default":
            ...
            if scapy3k.arch.SOLARIS:
                netmask = scapy3k.utils.atol(mask)
            elif "/" in dest:
                dest,netmask = dest.split("/")
                netmask = scapy3k.utils.itom(int(netmask))
            else:
                netmask = scapy3k.utils.itom((dest.count(".") + 1) * 8)
            dest += ".0"*(3-dest.count("."))
            dest = scapy3k.utils.atol(dest)
        if not "G" in flg:
            gw = '0.0.0.0'
        if netif is not None:
            ifaddr = scapy3k.arch.get_if_addr(netif)
...
def read_routes6():
    ...
    for l in f.readlines():
        ...
        if scapy3k.arch.NETBSD or scapy3k.arch.OPENBSD:
            lspl = l.split()
            d,nh,fl = lspl[:3]
        ...
        if '%' in nh:
            nh,dev = nh.split('%')
        if scapy3k.arch.LOOPBACK_NAME in dev:
            ...
            cset = scapy3k.utils6.construct_source_candidate_set(d, dp, devaddrs, scapy3k.arch.LOOPBACK_NAME)
