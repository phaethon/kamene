...
from scapy3k.error import *
import scapy3k.config

try:
    import matplotlib.pyplot as plt
    MATPLOTLIB = True
    if scapy3k.config.conf.interactive:
        plt.ion()
except ImportError:
...
if scapy3k.config.conf.use_netifaces:
  try:
    import netifaces
  except ImportError as e:
    log_loading.warning("Could not load module netifaces: %s" % e)
    scapy3k.config.conf.use_netifaces = False
...
if LINUX:
    from .linux import *
    if scapy3k.config.conf.use_winpcapy or scapy3k.config.conf.use_netifaces:
...
if scapy3k.config.conf.iface is None:
    scapy3k.config.conf.iface = LOOPBACK_NAME
