...
from scapy3k.utils import checksum, is_private_addr
from scapy3k.layers.l2 import ( BitEnumField,
                              BitField,
                              ByteEnumField,
                              ByteField,
                              ConditionalField,
                              ...
                              time,
                              warning)
from scapy3k.sendrecv import sr, sr1
from scapy3k.plist import PacketList, SndRcvList
from scapy3k.automaton import Automaton, ATMT

import scapy3k.as_resolvers
...
class TCP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            ...
            elif conf.ipv6_enabled and isinstance(self.underlayer, scapy3k.layers.inet6.IPv6) or isinstance(self.underlayer, scapy3k.layers.inet6._IPv6ExtHdr):
                ck = scapy3k.layers.inet6.in6_chksum(socket.IPPROTO_TCP, self.underlayer, p)
                p = p[:16] + struct.pack("!H", ck) + p[18:]
            else:
    ...
    def mysummary(self):
        ...
        elif conf.ipv6_enabled and isinstance(self.underlayer, scapy3k.layers.inet6.IPv6):
...
class UDP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            ...
            elif isinstance(self.underlayer, scapy3k.layers.inet6.IPv6) or isinstance(self.underlayer, scapy3k.layers.inet6._IPv6ExtHdr):
                ck = scapy3k.layers.inet6.in6_chksum(socket.IPPROTO_UDP, self.underlayer, p)
                p = p[:6] + struct.pack("!H", ck) + p[8:]
            else:
                warning("No IP underlayer to compute checksum. Leaving null.")
        return p
    ...
    def mysummary(self):
        ...
        elif isinstance(self.underlayer, scapy3k.layers.inet6.IPv6):
...
#        return g
    ...
    def make_graph(self, ASres=None, padding=0):
        ...
        ports_done = {}
        for s, r in self.res:
            r = r.getlayer(IP) or (conf.ipv6_enabled and r[scapy3k.layers.inet6.IPv6]) or r
            s = s.getlayer(IP) or (conf.ipv6_enabled and s[scapy3k.layers.inet6.IPv6]) or s
            ips[r.src] = None
            if TCP in s:
                ...
                trace_id = (s.src, s.dst, s.proto, 0)
            trace = rt.get(trace_id, {})
            ttl = conf.ipv6_enabled and scapy3k.layers.inet6.IPv6 in s and s.hlim or s.ttl
            if not (ICMP in r and r[ICMP].type == 11) and not (conf.ipv6_enabled and scapy3k.layers.inet6.IPv6 in r and scapy3k.layers.inet6.ICMPv6TimeExceeded in r):
                if trace_id in ports_done:
                    continue
                ports_done[trace_id] = None
                p = ports.get(r.src, [])
...
def mtr(target, dport=80, minttl=1, maxttl=30, stype="Random", srcport=50000, iface=None, l4=None, filter=None, timeout=2, verbose=None, gw=None, netproto="TCP", nquery=1, ptype=None, payload=b'', privaddr=0, rasn=1, **kargs):
    ...
    for t in target:
        #
        # Use scapy3k's 'Net' function to expand target...
